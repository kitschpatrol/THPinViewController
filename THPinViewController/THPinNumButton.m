...
- (instancetype)initWithNumber:(NSUInteger)number letters:(NSString *)letters {
  ...
  if (self) {
    ...
    if (letters) {
      ...
      _lettersLabel.font = [UIFont fontWithName:@"Libertad" size:(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? 11.0f : 9.0f];
      [contentView addSubview:_lettersLabel];
      [contentView addConstraints:
                       [NSLayoutConstraint constraintsWithVisualFormat:@"H:|[lettersLabel]|" options:0 metrics:nil views:@{
                         @"lettersLabel": _lettersLabel
                       }]];
      ...
      [contentView addConstraint:[NSLayoutConstraint constraintWithItem:_numberLabel
                                                              attribute:NSLayoutAttributeTop
                                                              relatedBy:NSLayoutRelationEqual
                                                                 toItem:contentView
                                                              attribute:NSLayoutAttributeTop
                                                             multiplier:1.0f
                                                               constant:numberLabelYCorrection]];
      // pin letter label to bottom
      [contentView addConstraint:[NSLayoutConstraint constraintWithItem:_lettersLabel
                                                              attribute:NSLayoutAttributeBottom
                                                              relatedBy:NSLayoutRelationEqual
                                                                 toItem:contentView
                                                              attribute:NSLayoutAttributeBottom
                                                             multiplier:1.0f
                                                               constant:lettersLabelYCorrection]];
    } else {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  UIColor *textColor = ([self.backgroundColorBackup isEqual:[UIColor clearColor]] ? [self.class averageContentColor] : self.backgroundColorBackup);
...
- (void)resetHighlight {
  [UIView animateWithDuration:0.3f
      ...
      animations:^{ self.backgroundColor = self.backgroundColorBackup; }
...
+ (UIColor *)averageContentColor {
  ...
  dispatch_once(&onceToken, ^{
      ...
      averageContentColor = [UIColor colorWithRed:data[2] / 255.0f green:data[1] / 255.0f blue:data[0] / 255.0f alpha:1.0f];
